...
func HandleAuthenticate(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	token, err := s.Authenticate(ctx, input.Email, input.Password)
